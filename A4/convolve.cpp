...
int main(int argc, char **argv) {
	...
	printf("==================================================\n");	
	printf("IR file: \n");
	printf("format: %i, channels: %i, sample rate: %d\n", impulse_header.audio_format, impulse_header.num_channels, impulse_header.sample_rate);
	...
	double *impulse_buf1 = new double[M];
	double *impulse_buf2 = new double[M];
	...
	if (impulse_bps == 1) {
		double sample;
		for (int i = 0; i < M; i++) {
			//printf("Reading into impulse_buf: %d/%d\n", i, M);
			fread(&sample, impulse_bps, 1, impulse_file);
			impulse_buf1[i] = sample;
			if (impulse_header.num_channels == 2) {
				fread(&sample, impulse_bps, 1, impulse_file);
				impulse_buf2[i] = sample;
			}
		}
	}
	...
	for (int n = 0; n < N; n++) {
		...
		for (int m = 0; m < M; m++) {
			output_buf1[n+m] += impulse_buf1[m] * input_buf1[n];
			output_buf2[n+m] += impulse_buf2[m] * input_buf2[n];
		}
	}
